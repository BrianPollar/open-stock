...
const uploadFiles = (req, res, next) => {
    ...
    makeupload(req, res, function (err) {
        if (err instanceof multer_1.default.MulterError) {
            back_logger_1.mainLogger.error('Multer UPLOAD ERROR', err);
            return res.status(500).send({ success: false });
        }
        else if (err) {
            // An unknown error occurred when uploading.
            back_logger_1.mainLogger.error('UNKWON UPLOAD ERROR', err);
            return res.status(500).send({ success: false });
        }
        return next();
    });
...
const appendBody = (req, res, next) => {
    ...
    const videoDirectory = path.join(stock_universal_local_1.stockUniversalConfig.envCfig.videoDirectory + '/' + companyId + '/');
    const photoDirectory = path.join(stock_universal_local_1.stockUniversalConfig.envCfig.photoDirectory + '/' + companyId + '/');
...
const saveMetaToDb = async (req, res, next) => {
    ...
    if (parsed.newPhotos) {
        const promises = parsed.newPhotos
            ...
            const newSaved = await newFileMeta.save().catch((err) => {
                back_logger_1.mainLogger.error('save error', err);
                reject(err);
            });
            resolve(newSaved);
        }));
    ...
    if (parsed.thumbnail) {
        ...
        if (newSaved instanceof mongoose_1.Error) {
            return res.status(500).send({ success: false });
        }
...
const deleteAllFiles = async (filesWithDir, directlyRemove = false) => {
    ...
    if (!directlyRemove) {
        await filemeta_model_1.fileMeta.updateMany({ _id: { $in: _ids } }, { $set: { isDeleted: true } });
    }
    else {
        await filemeta_model_1.fileMeta.deleteMany({ _id: { $in: _ids } });
    }
    if (filesWithDir && filesWithDir.length && directlyRemove) {
        const promises = filesWithDir
            .map((value /** : Ifilewithdir */) => new Promise(resolve => {
            back_logger_1.mainLogger.debug('deleting file', value.url);
            ...
            fs.unlink(nowpath, (err) => {
                if (err) {
                    back_logger_1.mainLogger.error('error while deleting file', err);
                    resolve(false);
                }
                else {
                    resolve(true);
...
const getOneFile = (req, res) => {
    ...
    back_logger_1.mainLogger.debug(`download, file: ${filename}`);
    const absolutepath = stock_universal_local_1.stockUniversalConfig.envCfig.absolutepath;
    ...
    return new Promise((resolve, reject) => {
        ...
        (0, background_removal_node_1.removeBackground)(fileLocation, config).then(async (blob) => {
            ...
            fs.writeFile(fileLocation, buffer, {}, (err) => {
                ...
                back_logger_1.mainLogger.debug('files saved');
                resolve(imageSrc);
            });
        });
    });
