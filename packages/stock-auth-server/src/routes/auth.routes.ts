...
import {
  ...
  sendTokenEmail,
  sendTokenPhone,
  setUserInfo, validateEmail,
  validatePhone
} from '../controllers/universial.controller';
import { user, userAuthSelect, userLean } from '../models/user.model';
import { Iaddress, Iauthresponse, IauthresponseObj, Iauthtoken, Ibilling, Icustomrequest, Isuccess, Iuser, Iuserperm } from '@open-stock/stock-universal';
...
export const userLoginRelegator = async(req: Request, res: Response) => {
  ...
  if (from === 'admin') {
    ...
    const token = generateToken(
      userInfo, '1d', req.app.locals.stockAuthServer.lAuth.jwtSecret);
...
authRoutes.post('/login/admin', async(req, res, next) => {
  ...
  const isAdmin = await checkIfAdmin(
    emailPhone,
    passwd,
    req.app.locals.stockAuthServer.aAuth.processadminID,
    req.app.locals.stockAuthServer.aAuth.password);
  if (isAdmin.success === true) {
    const userInfo: Iauthtoken = setUserInfo(
      'admin', isAdmin.user?.permissions as Iuserperm);
    const token = generateToken(
      userInfo, '1d', req.app.locals.stockAuthServer.lAuth.jwtSecret);
    isAdmin.token = token;
...
authRoutes.post('/signup', async(req, res) => {
  ...
  return newUser.save().then((newuser) => {
    const done = async() => {
      ...
      if (isPhone) {
        response = await sendTokenPhone(
          newuser);
      } else {
        response = await sendTokenEmail(req.app,
          newuser, type, req.app.locals.stockAuthServer.localEnv.appOfficialName);
      }
      if (!response.success) {
        (newuser as any).remove(); // TODO
        return res.status(200).send(response);
      }
...
authRoutes.post('/confirm', async(req, res) => {
  ...
  if (how === 'phone') {
    ...
    response = await validateEmail(_id,
      type,
      'signup',
      verifycode, null);
  }
  ...
  const urId = makeUrId(Number(count[0]?.urId || '0'));
    ...
    if (errResponse) {
      ...
      return res.status(200).send({
        success: true,
        user: (nUser as any).toAuthJSON() // TODO
      });
    }
...
authRoutes.post('/adduser', requireAuth, roleAuthorisation('users'), async(req, res) => {
  ...
  if(!response.err && savedUser) {
    ...
    await sendTokenEmail(req.app,
      savedUser, type, req.app.locals.stockAuthServer.localEnv.appOfficialName);
  }
  return res.status(status).send(response);
});
