...
import { createMockPayment, createMockPaymentRelated, createMockPayments, createMockInvoiceRelated, createMockOrder } from '../../../../tests/mocks';
...
describe('Payment', () => {
  ...
  it('#getPayments static should get Payments array', async() => {
    ...
    const list = await Payment.getPayments('/', 0, 0);
  ...
  it('#searchPayments static should search Payments and return matching items array', async() => {
    ...
    const list = await Payment.searchPayments('/', '/');
    expect(typeof list).toEqual('object');
    expectTypeOf(list).toEqualTypeOf<Payment[]>([]);
    expect(lSpy).toHaveBeenCalled();
  });
  ...
  it('#getOnePayment static should get one Payment', async() => {
    ...
    const one = await Payment.getOnePayment('urId');
  ...
  it('#deletePayments static should delete many Payments', async() => {
    ...
    const deleted = await Payment.deletePayments([]);
