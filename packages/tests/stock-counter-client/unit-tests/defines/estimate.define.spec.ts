...
import { createMockInvoiceRelated, createMockEstimate, createMockEstimates } from '../../../../tests/mocks';
...
describe('Estimate', () => {
  ...
  it('#getEstimates static should get Estimates array', async() => {
    ...
    const list = await Estimate.getEstimates('/', 0, 0);
  ...
  it('#getOneEstimate static should get one Estimate', async() => {
    ...
    const one = await Estimate.getOneEstimate(1);
  ...
  it('#addEstimate static should add one Estimate', async() => {
    ...
    const added = await Estimate.addEstimate(createMockEstimate(), createMockInvoiceRelated());
  ...
  it('#deleteEstimates static should delete many Estimates', async() => {
    ...
    const deleted = await Estimate.deleteEstimates([]);
  ...
  /* it('#updateEstimate should update Estimate', async() => {
    const lSpy = vi.spyOn(StockCounterClient.ehttp, 'makePut').mockImplementationOnce(() => of({ success: true }));
    const updated = await instance.updateEstimate(createMockEstimate(), createMockInvoiceRelated());
  ...
  it('#updateEstimatePdt should update estimate product', async() => {
    const lSpy = vi.spyOn(StockCounterClient.ehttp, 'makePut').mockImplementationOnce(() => of({ success: true }));
    const updated = await instance.updateEstimatePdt(createMockInvoiceRelated());
