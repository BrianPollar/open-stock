...
import { Icompany, IdeleteCredentialsLocalUser, IfileMeta, IcompanyPerm, IpermProp, Iuser, Iuserperm, Iaddress } from '@open-stock/stock-universal';
...
export const createPermProp = (state = 'normal'): IpermProp => {
  return {
    create: state !== 'normal',
    read: state !== 'normal',
    update: state !== 'normal',
...
export const createMockCompanyPerm = (active = true): IcompanyPerm => {
  return {
    active
  };
};
...
export const createMockFileMeta = (allowOptionals = false): IfileMeta => {
  const compulObjs = {
...
export const createMockDeleteCredentialsLocalUser = (): IdeleteCredentialsLocalUser => {
  return {
