...
import { createMockItemDecoy } from '../../../mocks';
...
describe('ItemDecoyRoutes', () => {
  ...
  beforeAll(async() => {
    ...
    app.use(apiUrl, itemDecoyRoutes);
    server = app.listen(5021, () => {
  ...
  it('should create itemdecoy', async() => {
    const body = {
      itemdecoy: createMockItemDecoy()
    };
    const res = await request(app).post(apiUrl + '/create/automatic')
