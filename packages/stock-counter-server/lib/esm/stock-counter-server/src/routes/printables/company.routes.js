...
import { appendBody, deleteFiles, fileMetaLean, makeUrId, requireAuth, roleAuthorisation, saveMetaToDb, stringifyMongooseErr, uploadFiles, verifyObjectId } from '@open-stock/stock-universal-server';
 ...
 */
export const companyLoginRelegator = async (req, res) => {
    ...
    const permissions = {
        ...
        faqs: permProps,
        videos: permProps,
        printables: permProps,
        buyer: permProps
    };
    // delete user.password; //we do not want to send back password
    const userInfo = setUserInfo(foundCompany._id, // 'admin',
...
companyAuthRoutes.get('/authexpress/:companyIdParam', requireAuth, async (req, res) => {
    ...
    const userInfo = setUserInfo('admin', permissions, foundCompany._id, {
        active: !foundCompany.blocked
    });
    const token = generateToken(userInfo, '1d', getStockAuthConfig().authSecrets.jwtSecret);
    const nowResponse = {
...
companyAuthRoutes.post('/updatecompanybulkimg/:companyIdParam', requireAuth, roleAuthorisation('users', 'update'), uploadFiles, appendBody, saveMetaToDb, async (req, res) => {
    const { companyId } = req.user;
    const { companyIdParam } = req.params;
    const queryId = companyId === 'superAdmin' ? companyIdParam : companyId;
    const updatedUser = req.body.user;
...
companyAuthRoutes.put('/deleteimages/:companyIdParam', requireAuth, roleAuthorisation('users', 'delete'), deleteFiles, async (req, res) => {
