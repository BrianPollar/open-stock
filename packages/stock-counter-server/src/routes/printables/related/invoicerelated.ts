...
import {
  addParentToLocals, handleMongooseErr, mainLogger, verifyObjectId, verifyObjectIds
...
import {
  ...
  invoiceRelatedLean,
  invoiceRelatedMain
} from '../../../models/printables/related/invoicerelated.model';
 ...
 */
export const updateInvoiceRelatedPayments = async(payment: Ireceipt): Promise<Isuccess & { _id?: string }> => {
  ...
  const invoiceRel: IinvoiceRelated = {
    ...related,
...
export const updateInvoiceRelated = async(
  res,
  ...
  const foundInvoiceRel: IinvoiceRelated = {
    ...related,
    billingUserId: related.billingUserId.toString()
  };
...
export const relegateInvRelatedCreation = async(
  ...
  if (!found || invoiceRelated.creationType === 'solo') {
    ...
    const savedRes = await newInvRelated.save().catch((err: Error) => err);
    ...
    const stn = await getCurrentNotificationSettings(companyId) as any;
...
export const makeInvoiceRelatedPdct = (
  invoiceRelated: Required<TinvoiceRelated | IinvoiceRelated>,
  user: Iuser, createdAt?: Date, extras: object = {}
): IinvoiceRelated => {
  // TODO later
  ...
  if (extras && (extras as { updatedAt: Date }).updatedAt) {
    updatedAt = (extras as { updatedAt: Date }).updatedAt;
  }
  ...
  return {
    ...
    billingUserPhoto: user?.profilePic as string,
...
export const deleteManyInvoiceRelated = async(_ids: string[], companyId: string) => {
 ...
 */
export const deleteAllLinked = async(
  ...
  let changedStage: TestimateStage = 'estimate';
 ...
 */
const updateRelatedStage = async(_id: string, stage: TestimateStage, companyId: string) => {
  const related = await invoiceRelatedMain
    .findOne({ _id })
    .lean();
...
export const updateItemsInventory = async(related: string | IinvoiceRelated) => {
  let relatedObj: IinvoiceRelated | null;
...
export const getPaymentsTotal = async(payments: string[]) => {
  ...
  return allPromises
    .filter(val => val !== null)
    .reduce((total, payment) => total + (payment as any).amountRcievd, 0);
};
...
/**
   ...
   */
export const updateCustomerDueAmount = async(
  userId: string,
  amount: number,
  reduce: boolean
) => { // TODO currency conversion needed here
