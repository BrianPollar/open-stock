...
• isPublished: The boolean value
representing if the product is published or not. (Boolean)
...
•
if the matching product exists, it should
validate if the product can be published based on the following criteria in the same order they are mentioned below:
。 CRITERIA 1: check if the mrp property of the matching product is greater than
equal to the selling price of the product 。 CRITERIA 2: check if the stock quantity of the product is greater than 0.
...
o if only CRITERIA 2 fails, the response body should be an array containing the message 'Stock count is O' at the Oth index.
o if both CRITERIA 1 and CRITERIA 2 fail, the response body should be an array
containing the messages 'MRP should be less than equal to the Price' at the Oth index, and the message 'Stock count is O' at the 1st index.
• if all the criterias pass, the matching product's isPublished field is be set to true and the response code is 204 with no response body. • you can assume that the ID passed to the request will always be valid
