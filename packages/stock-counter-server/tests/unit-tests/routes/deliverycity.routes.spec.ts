...
import { createMockDeliveryCity } from '../../../mocks';
...
import { deliverycityRoutes } from '../../../../stock-counter-server/src/routes/deliverycity.routes';
...
describe('deliverycity', () => {
  ...
  beforeAll(async() => {
    ...
    app.use(apiUrl, deliverycityRoutes);
    server = app.listen(5017, () => {
  ...
  it('should create one given the right data type', async() => {
    const body = {
      deliverycity: createMockDeliveryCity()
    };

    delete body.deliverycity['_id'];
