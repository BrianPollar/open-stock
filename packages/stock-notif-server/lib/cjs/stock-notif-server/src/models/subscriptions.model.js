...
const createSubscriptionModel = async (dbUrl, dbOptions, main = true, lean = true) => {
    ...
    if (main) {
        exports.subscriptionMain = database_1.mainConnection.model('Subscription', subscriptionSchema);
    }
    if (lean) {
        exports.subscriptionLean = database_1.mainConnectionLean.model('Subscription', subscriptionSchema);
    }
};
