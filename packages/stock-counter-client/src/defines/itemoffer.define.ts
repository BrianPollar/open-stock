import {
  DatabaseAuto, IdataArrayResponse, IdeleteMany, IfilterProps, IitemOffer, IsubscriptionFeatureState, Isuccess
...
export class ItemOffer extends DatabaseAuto {
  ...
  static async getAll(
    type: string, // TODO union here
    offset = 0,
    limit = 20
  ) {
    const observer$ = StockCounterClient.ehttp
      .makeGet<IdataArrayResponse<IitemOffer>>(`/itemoffer/all/${type}/${offset}/${limit}`);
  ...
  static async filterAll(filter: IfilterProps) {
    const observer$ = StockCounterClient.ehttp
      .makePost<IdataArrayResponse<IitemOffer>>('/itemoffer/filter', filter);
    const offers = await lastValueFrom(observer$);
  ...
  static async getOne(urIdOr_id: string) {
    const observer$ = StockCounterClient.ehttp
      .makeGet<IitemOffer>(`/itemoffer/one/${urIdOr_id}`);
  ...
  static removeMany(vals: IdeleteMany) {
    const observer$ = StockCounterClient.ehttp
      .makePut<Isuccess>('/itemoffer/delete/many', vals);
  ...
  remove() {
    const observer$ = StockCounterClient.ehttp.makeDelete<Isuccess>(`/itemoffer/delete/one/${this._id}`);
