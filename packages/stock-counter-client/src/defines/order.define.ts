...
export class Order extends PaymentRelated {
   ...
   */
  static async searchOrders(searchterm: string, searchKey: string) {
    const observer$ = StockCounterClient.ehttp.makePost('/order/search', { searchterm, searchKey });
   ...
   */
  static async deleteOrders(credentials: IdeleteCredentialsPayRel[]) {
    const observer$ = StockCounterClient.ehttp.makePut('/order/deletemany', { credentials });
   ...
   */
  async appendDelivery(status: TorderStatus) {
    const observer$ = StockCounterClient.ehttp
