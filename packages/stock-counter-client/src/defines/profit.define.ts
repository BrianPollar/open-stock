...
export class Profit extends DatabaseAuto {
   ...
   */
  static async getProfits(url = 'getall', offset = 0, limit = 0): Promise<Profit[]> {
    const observer$ = StockCounterClient.ehttp.makeGet(`profit/${url}/${offset}/${limit}`);
   ...
   */
  static async getOneProfit(id: string): Promise<Profit> {
   ...
   */
  static async addProfit(vals: Iprofit): Promise<Isuccess> {
    const observer$ = StockCounterClient.ehttp.makePost('/profit/create', vals);
   ...
   * @returns {Promise<Isuccess>} - A Promise that resolves to a success message.
   */
  static async deleteProfits(ids: string[], filesWithDir: any, url: string): Promise<Isuccess> {
    const observer$ = StockCounterClient.ehttp.makePut(`${url}`, { ids, filesWithDir });
    return await lastValueFrom(observer$) as Isuccess;
  }
   ...
   * @returns {Promise<Isuccess>} - A Promise that resolves to a success message.
   */
  async updateProfit(vals: Iprofit): Promise<Isuccess> {
    const observer$ = StockCounterClient.ehttp.makePut('profit/update', vals);
    return await lastValueFrom(observer$) as Isuccess;
  }
}
