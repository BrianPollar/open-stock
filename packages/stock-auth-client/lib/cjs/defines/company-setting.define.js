...
class CompanySetting extends stock_universal_1.DatabaseAuto {
    /**
    ...
    static async getCompanySetting(companyId, offset = 0, limit = 20) {
        const observer$ = stock_auth_client_1.StockAuthClient.ehttp.makeGet(`/companysetting/getall/${offset}/${limit}/${companyId}`);
    ...
    static async getOneCompanySetting(companyId, urId) {
        const observer$ = stock_auth_client_1.StockAuthClient.ehttp.makeGet(`/companysetting/getone/${urId}/${companyId}`);
    ...
    static async addCompanySetting(companyId, vals, files) {
        let added;
        if (files && files[0]) {
            const observer$ = stock_auth_client_1.StockAuthClient.ehttp.uploadFiles(files, `/companysetting/addcompanyimg/${companyId}`, vals);
            added = await (0, rxjs_1.lastValueFrom)(observer$);
        }
        else {
            const observer$ = stock_auth_client_1.StockAuthClient.ehttp.makePost('/companysetting/addcompany', vals);
            added = await (0, rxjs_1.lastValueFrom)(observer$);
        }
        return added;
    }
    ...
    static async deleteCompanySetting(companyId, ids, filesWithDir) {
        const observer$ = stock_auth_client_1.StockAuthClient.ehttp.makePut(`/companysetting/deletemany/${companyId}`, { ids, filesWithDir });
